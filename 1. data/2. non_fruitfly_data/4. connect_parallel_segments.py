...
path_source = '4. junction_networks/'
path_save = '5. junction_networks_merged_parallel_segments/'
...
for name in name_list:
    ...
    for path_key, points in new_path_dict.items():
        for i in range(len(points)-1):
            ...
            row = list(points[i][:3]) + [str(int(points[i][3]))] + [points[i][4]] + list(points[i+1][:3]) + [str(int(points[i+1][3]))] + [points[i+1][4]] + list(path_key) + [stopping_step_dict[path_key[0]]]
            row_list.append(row)
    
    new_df = pd.DataFrame(row_list, columns=['x_1', 'y_1', 'z_1', 'pt_id_1', 'radius_1', 'x_2', 'y_2', 'z_2', 'pt_id_2', 'radius_2', 'path_id', 'source', 'target','parallel_merges_per_path_id'])

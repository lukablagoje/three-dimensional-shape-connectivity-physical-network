...
for name in name_list:
    ...
    path_save = '4. junction_networks/'
    
    # Load the segments data for the current structure
    segments_df = pd.read_csv(path_source + name + '.segments.csv', index_col=0)
    
    # Extract the skeleton_healing_done column to merge later
    healing_done = segments_df[['pt_id1', 'pt_id2', 'segment_healing_done']].copy()

    # Label paths in the segment data, which includes identifying path start and end points, and edge lists for each path
    path_labels, path_endpoints, path_edgelists = label_paths(segments_df[['pt_id1', 'pt_id2']].astype(int).drop_duplicates().values)
    ...
    df_dict = {'pt_id_1': [], 'pt_id_2': [], 'path_id': [], 'source': [], 'target': [], 'segment_healing_done': []}
    ...
    for path_id in sorted_path_edgelists:
        for edge in sorted_path_edgelists[path_id]:
            if edge[0] != edge[1]:  # Ensure the edge is not a loop back to the same point
                ...
                healing_status_row = healing_done[((healing_done['pt_id1'] == edge[0]) & (healing_done['pt_id2'] == edge[1])) | ((healing_done['pt_id1'] == edge[1]) & (healing_done['pt_id2'] == edge[0]))]
                if not healing_status_row.empty:
                    df_dict['segment_healing_done'].append(healing_status_row['segment_healing_done'].values[0])
                else:
                    df_dict['segment_healing_done'].append('not_available')  # Default value if no match is found
    ...
    df_dict = {'pt_id_1': [], 'x_1': [], 'y_1': [], 'z_1': [], 'radius_1': [], 'pt_id_2': [], 'x_2': [], 'y_2': [], 'z_2': [], 'radius_2': [], 'path_id': [], 'source': [], 'target': [],'segment_healing_done':[]}
    ...
    for i, row in df_segment_labels.iterrows():
        ...
        df_dict['segment_healing_done'].append(row['segment_healing_done'])
